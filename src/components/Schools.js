import React from 'react';

export default class Schools extends React.Component {
    render() {
        return (
<div id="schoolsPage">
<h2>International Medical Graduates licensure process: </h2>

<h5>1. Document Source verification</h5>
<ul>
<li>Medical Degree - listed on World Directory of Medical Schools</li>
<li>Transcript</li>
<li>Medical license from home country</li>
<li>Passport</li>
</ul>
<p>For application go to <a href="https://physiciansapply.ca">physiciansapply.ca</a></p>
<h5>2. Examination</h5>
<p>IMGs are expected to write a series of examination and attend Clinical assessment rotation</p>
<ul>
<li>Medical Counsel of Canada Evaluating Examination(MCCEE)</li>
<li>NAC-OSCE</li>
<li>Clinical assessment rotation</li>
<li>Medical Counsel of Canada Qualifying Examination (QE-1)</li>
<li>Medical Counsel of Canada Qualifying Examination (QE-2)</li>
</ul>
<p>For application go to Medical Counsel of Canada <a href="https://mcc.ca/">mcc.ca</a></p>
<h5>3. Application for residency (Post graduate) matching program</h5>
<p>Application on Canadian Resident Matching Service takes place every year</p>
<p>There are two programs IMGs can apply</p>
<ul>
<li>First Iteration</li>
<li>Second iteration</li>
</ul>
<p>Here is application process (Beginning of program application - September):</p>
<ul>
<li>Program selection</li>
<li>Submission of translation requests</li>
<li>Document transfers from <a href="https://physiciansapply.ca">physiciansapply.ca</a></li>
<li>Letter of Recommendation sent by mail</li>
<li>Document assignments, file review, national Interview, ranking, rank order list</li>
<li>Matching - February</li>
</ul>
For more information go to <a href="www.carms.ca">carms.ca</a>
</div>
        );
    }
}
